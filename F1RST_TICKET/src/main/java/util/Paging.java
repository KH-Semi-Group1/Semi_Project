...
public class Paging {
	...
	public Paging(int totalCount, int curPage, int listCount) {
		setTotalCount(totalCount);
		setCurPage(curPage);
		setListCount(listCount); // 화면에 보여질 개수 지정하기
	...
	private void makePaging() {
		...
		// 기본값 설정
		if(curPage == 0) this.curPage = 1; // 첫 페이지를 기본 페이지로 설정
		if(listCount == 0) this.listCount = 10; // 화면에 보여질 게시글의 수를 10개로 설정
