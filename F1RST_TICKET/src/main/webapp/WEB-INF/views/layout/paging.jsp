...
<%-- 		<%	} %> --%>
		...
		<li><a href="./list?curPage=<%=paging.getStartPage() - paging.getPageCount() %>">&laquo;</a>
		<%	} %>
		
		<%-- 이전 페이지로 이동 --%>
		<%	if(paging.getCurPage() > 1) { %>
		<li><a href="./list?curPage=<%=paging.getCurPage()-1 %>">&lt;</a>
		<%	} %>
		...
		<li><a href="./list?curPage=<%=paging.getStartPage() + paging.getPageCount() %>">&raquo;</a>
		<%	} %>
